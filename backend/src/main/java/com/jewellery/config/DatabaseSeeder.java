...
public class DatabaseSeeder implements CommandLineRunner {
    ...
    private void seedUsers() {
        try {
            ...
            try { jdbcTemplate.execute("UPDATE `users` SET `role` = 'USER' WHERE `role` IS NULL OR `role` = '' OR LOWER(`role`) = 'user'"); } catch (Exception ignored) {}
            try { jdbcTemplate.execute("UPDATE `users` SET `role` = 'ADMIN' WHERE LOWER(`role`) = 'admin'"); } catch (Exception ignored) {}
